 ...
 const formatDate=(index)=>{
    ...
    return(
      ...
      <TableBody className='overflow-x-auto'>
        {blog?.map((item,index) => (
          <TableRow key={index}>
            <TableCell className="flex gap-4 items-center"><img src={item.thumbnail} className="w-20 rounded-md hidden md:block" alt=""/>
            <h1 onClick={()=>navigate(`/blogs/${item._id}`)} className="hover:underline cursor-pointer w-[58px] md:w-full truncate">{item.title}</h1>
            </TableCell>
            ...
            <TableCell className="text-center">
               ...
               <DropdownMenuTrigger><BsThreeDotsVertical/></DropdownMenuTrigger>
                <DropdownMenuContent>
             <DropdownMenuItem onClick={()=>navigate(`/dashboard/create-blog/${item._id}`)}> <Edit/> Edit</DropdownMenuItem>
